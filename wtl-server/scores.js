	...
	try {
		...
		if (auth.verifyToken(token, uid)) {
			...
			for (const score of scoresreq) {
				...
				// Check if judgment counts are all non negative integers (thanks mayflower)
				if ((w1 < 0) || (w2 < 0) || (w3 < 0) || (w4 < 0) || (w5 < 0) || (w6 < 0) || (w7 < 0) || (holdsHit < 0) || (minesHit < 0)) {
					console.log("Score for " + folderTitle + " contains non-negative integer judgment counts");
					checks = false;
				}

				// Do these checks only if there is a valid chart, otherwise the server just locks up
				if (curChart) {			
